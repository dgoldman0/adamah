# Prototype Theory Based Exam

* **25 True/False** (Q1–25)
* **50 Multiple Choice** (Q26–75)
* **25 Fill in the Blank** (Q76–100)

---

## 🔹 SECTION I – TRUE / FALSE (25 QUESTIONS)

Mark each statement **True (T)** or **False (F).**

1. A system can be defined by any boundary the observer chooses, as long as that boundary is applied consistently during analysis.

2. If two components never touch physically, they cannot influence each other within the same system.

3. Emergent behavior can arise even when every individual part behaves in a simple and predictable way.

4. A system with more degrees of freedom is always easier to control than one with fewer.

5. Feedback can stabilize a system or destabilize it, depending on how it is arranged.

6. A small design change that does not alter any component alone can still alter the behavior of the whole system.

7. Path dependence means that the order of changes made to a system can affect its final state.

8. Redundancy in a system always increases reliability.

9. A mental model of a device can be useful even if it is incomplete, as long as it matches reality where it matters for the task.

10. Latent failure modes exist even when a system appears to be functioning correctly.

11. Nonlinear systems can have tipping points where small changes in input produce large changes in output.

12. All stable systems must be in perfect static equilibrium.

13. Interfaces between subsystems are often the most fragile points in a design.

14. Robust systems tend to handle variation and noise without large changes in behavior.

15. Sensitivity is desirable in systems that measure or detect small effects.

16. A design that optimizes one performance metric may reduce performance in another metric.

17. The environment around a system can be treated as part of the system if that makes reasoning easier.

18. A single component can simultaneously carry mechanical, electrical, and informational roles.

19. Model friction increases when observed behavior consistently surprises the tinkerer’s expectations.

20. A system that is perfectly predictable under test conditions will remain perfectly predictable in every real-world context.

21. All feedback loops contain at least one delay or inertia.

22. Overconstrained mechanical structures may jam or bind when tolerances stack unfavorably.

23. Hidden coupling between subsystems often appears as rare, hard-to-reproduce failures.

24. Design slack (extra capacity, space, or tolerance) can act as a buffer against uncertainty.

25. Every modification to a system involves a trade-off, even if that trade-off is subtle or delayed.
...
70. **Decoupling** subsystems usually:
    ...
    b) Reduces the spread of disturbances
    c) Eliminates interfaces entirely
    d) Guarantees perfect performance

71. A tinkerer who **overfits** their model to a single scenario risks:
    a) A model that generalizes poorly to new conditions
    b) A model that is always valid
    c) Ignoring data from the known scenario
    d) Removing all complexity

72. **Resonance** in a system typically results from:
    a) Matching a natural frequency with a periodic input
    b) Minimizing all sources of energy
    c) Removing all slack
    d) Perfect symmetry

73. Designing to **avoid resonance** often involves:
    a) Aligning frequencies exactly
    b) Randomizing or shifting natural frequencies and excitations
    c) Increasing feedback gain
    d) Reducing damping

74. **Information flow** in a system refers to:
    a) Only digital signals
    b) Any transmission of state or conditions that influence decisions
...
## 🔹 SECTION III – FILL IN THE BLANK (25 QUESTIONS)

Write the word or short phrase that best completes each statement.

76. A conceptual line separating “inside” from “outside” during analysis is called a system __________.

77. Behavior that arises from interactions among parts, rather than from any part alone, is called __________ behavior.

78. An independent way in which a mechanical assembly can move is a __________ of freedom.

79. A limit on motion or behavior within a system is called a __________.

80. A specific way in which a system can fail is known as a failure __________.

81. When a system’s current behavior depends on its history, it is described as __________.

82. The mismatch between a tinkerer’s mental model and a system’s real behavior can be called model __________.

83. The tendency of a system to drift toward disorder, wear, and uncertainty relates to the concept of __________.

84. A modification that can be undone with little effort has high __________.

85. When the final outcome depends on the sequence of changes, the system exhibits __________ dependence.

86. Extra capacity, space, or tolerance intentionally included in a design can be described as design __________.

87. The organized description of what a module expects and provides at its boundary is often called an interface __________.

88. Trade-offs appear because improving one performance metric can __________ another.

89. Hidden geometric relationships or stiffness paths that shape behavior belong to a system’s latent __________.

90. A condition where small differences in initial state lead to large differences in outcome is often described as __________ sensitivity.

91. A predictable yet complex system whose behavior highly depends on initial conditions is often called __________.

92. A rule of thumb that guides design or troubleshooting, without guaranteeing optimality, is known as a __________.

93. When two or more events’ timing relative to each other determines the outcome, a __________ condition can occur.

94. The exchange of signals, measurements, or state that influences decisions inside a system is called __________ flow.

95. When parts of a system influence one another strongly, the system is said to have high __________.

96. Organizing complexity into stacked levels with defined interfaces is called abstraction __________.

97. A system that maintains function under variation is often described as __________.

98. A system that responds strongly to small changes in input is described as highly __________.

99. Modifications that permanently alter the original state and are hard to undo are described as __________ changes.

100. The point at which gradual change leads to a sudden shift in behavior is often called a __________ point.
